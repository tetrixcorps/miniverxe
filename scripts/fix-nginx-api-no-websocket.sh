...
# Fix Nginx /api location - Remove WebSocket headers (they break API routes)
# WebSocket headers should only be in root location, not API routes
...
echo "🔧 Fixing Nginx /api location - Removing WebSocket headers..."
echo ""
...
echo ""
echo "3️⃣ Fixing /api location block (removing WebSocket headers)..."
python3 << 'PYTHON'
import re
...
def replace_with_api_block(match):
    ...
    # API location block WITHOUT WebSocket headers
    api_block = f'''{indent}# API routes - proxy to frontend container
{indent}# NOTE: No WebSocket headers here - they break API routes
{indent}location /api {{
...
{indent}    proxy_set_header X-Forwarded-Port $server_port;
...
print("✅ Config fixed - WebSocket headers removed from /api location")
PYTHON
...
if [ $? -eq 0 ]; then
    ...
    if echo "$RESPONSE2" | grep -q '"success":true'; then
        ...
        ssh -i $DROPLET_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            root@$DROPLET_IP "tail -20 /var/log/nginx/error.log | grep -E 'proxy|upstream|api' || echo 'No recent errors'"
    fi
