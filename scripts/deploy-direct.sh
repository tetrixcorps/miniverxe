...
# TETRIX Direct Deployment Script using doctl
# This script uses doctl to execute commands directly on the droplet

set -e  # Exit on any error

echo "🚀 Starting TETRIX Direct Deployment using doctl"
echo "==============================================="
...
# Upload package to droplet using doctl
upload_package() {
    print_status "Uploading package to droplet..."
    
    # Use doctl to upload the package
    doctl compute droplet-action ssh "$DROPLET_NAME" --ssh-command "mkdir -p $APP_DIR" || true
    
    # Upload the package using scp
    scp /tmp/tetrix-deployment.tar.gz root@$DROPLET_IP:/tmp/ || {
        print_error "Failed to upload package. Please check SSH key configuration."
        print_status "You may need to:"
        print_status "1. Add your SSH key to the droplet"
        print_status "2. Configure SSH agent: ssh-add ~/.ssh/id_rsa"
        print_status "3. Or use: ssh-copy-id root@$DROPLET_IP"
        exit 1
    }
...
setup_on_droplet() {
    ...
    # Extract package and setup
    doctl compute droplet-action ssh "$DROPLET_NAME" --ssh-command "
        cd $APP_DIR
...
EOF
    " || {
        print_error "Failed to setup application on droplet"
        exit 1
    }
...
setup_docker() {
    ...
    " || {
        print_error "Failed to setup Docker on droplet"
        exit 1
    }
...
deploy_application() {
    ...
    " || {
        print_error "Failed to deploy application on droplet"
        exit 1
    }
...
main() {
    echo "🚀 TETRIX Direct Deployment using doctl"
    echo "======================================="
    ...
    echo ""
    echo "To check logs: doctl compute droplet-action ssh $DROPLET_NAME 'cd $APP_DIR && docker-compose -f docker-compose.production.yml logs'"
    echo "To restart: doctl compute droplet-action ssh $DROPLET_NAME 'cd $APP_DIR && docker-compose -f docker-compose.production.yml restart'"
    echo "To stop: doctl compute droplet-action ssh $DROPLET_NAME 'cd $APP_DIR && docker-compose -f docker-compose.production.yml down'"
}
