...
# Move /api location block from HTTP server to HTTPS server block
...
echo "🔧 Moving /api block to HTTPS server block..."
echo ""
...
for i, line in enumerate(new_lines):
    if 'listen 443' in line:
        # Find the server { before this line
...
# Find location / in HTTPS block (after https_start)
location_root_idx = -1
...
print(f"✅ Added /api block in HTTPS server block (before line {location_root_idx + len(api_block)})")
PYTHON
