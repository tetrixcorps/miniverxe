...
/**
 * Test Code Academy SinchChatLive Integration - Fixed Version
...
async function testCodeAcademyFrontend() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (html.includes('Code Academy') && html.includes('AI-Powered')) {
        logTest('Code Academy Frontend', 'PASS', 'Frontend accessible with correct content');
        return true;
      } else {
...
// Test 9: Code Academy Route Test
async function testCodeAcademyRoute() {
  console.log('\n🔍 Testing Code Academy Route...');
  
  try {
    const response = await fetch(`${BASE_URL}/code-academy`);
    if (response.ok) {
      const html = await response.text();
      if (html.includes('Code Academy') && html.includes('AI-Powered') && html.includes('SHANGO AI')) {
        logTest('Code Academy Route', 'PASS', 'Route accessible with all expected content');
        return true;
      } else {
        logTest('Code Academy Route', 'FAIL', 'Missing expected content');
        return false;
      }
    } else {
      logTest('Code Academy Route', 'FAIL', `HTTP ${response.status}`);
      return false;
    }
  } catch (error) {
    logTest('Code Academy Route', 'FAIL', error.message);
    return false;
  }
}
...
async function runAllTests() {
  console.log('🚀 Starting Code Academy SinchChatLive Integration Tests - Fixed Version...\n');
  ...
  // Test 9: Code Academy route
  await testCodeAcademyRoute();
  ...
  console.log('1. SinchChatLive: Primary AI system (working with proper environment variables)');
  console.log('2. Ollama: Fallback AI system (working)');
  console.log('3. Code Academy: Frontend integration (working with dedicated route)');
