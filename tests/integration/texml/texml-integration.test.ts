...
describe('Advanced TeXML Features', () => {
  ...
  test('should generate voicemail TeXML', () => {
    ...
    expect(texml).toContain('voicemail-status');
  });
  ...
  test('should generate queue TeXML', () => {
    ...
    expect(texml).toContain('test-queue');
...
describe('TeXML Analytics', () => {
  ...
  test('should calculate call metrics', () => {
    const testAnalytics = new TeXMLAnalytics();
    
    // Track multiple events
    testAnalytics.trackCallEvent('call.initiated', { industry: 'healthcare', scenario: 'patient_intake' });
    testAnalytics.trackCallEvent('call.initiated', { industry: 'legal', scenario: 'client_intake' });
    testAnalytics.trackCallEvent('call.hangup', { industry: 'healthcare', duration: 120 });
    testAnalytics.trackCallEvent('call.hangup', { industry: 'legal', duration: 180 });

    const metrics = testAnalytics.getCallMetrics('1h');
    expect(metrics.totalCalls).toBeGreaterThanOrEqual(1);
    expect(metrics.completedCalls).toBeGreaterThanOrEqual(1);
    expect(metrics.industryBreakdown).toBeDefined();
    expect(metrics.averageCallDuration).toBeGreaterThan(0);
  });
  ...
  test('should get top scenarios', () => {
    ...
    const metrics = testAnalytics.getCallMetrics('1h');
    expect(metrics.topScenarios.length).toBeGreaterThan(0);
    expect(metrics.topScenarios[0]).toHaveProperty('scenario');
    expect(metrics.topScenarios[0]).toHaveProperty('count');
    expect(metrics.topScenarios[0].count).toBeGreaterThan(0);
  });
});
