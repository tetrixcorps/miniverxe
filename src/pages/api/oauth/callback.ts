...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    // In production, decode state to get userId and integrationId
    ...
    const redirectUrl = url.searchParams.get('redirect_uri') || '/integrations/success';
    
    return new Response(null, {
      status: 302,
      headers: {
        Location: `${redirectUrl}?success=true&integration_id=${result.integrationId}`,
      },
    });
  } catch (error) {
    console.error('OAuth callback error:', error);
    
    const redirectUrl = url.searchParams.get('redirect_uri') || '/integrations/error';
    
    return new Response(null, {
      status: 302,
      headers: {
        Location: `${redirectUrl}?error=${encodeURIComponent(
